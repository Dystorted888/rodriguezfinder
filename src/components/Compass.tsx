...
function useVibrateCues(enabled: boolean, distanceM: number | null) {
  ...
  useEffect(() => {
    ...
    const doVibe = () => {
      if (d <= 5) navigator.vibrate?.([pulse, 90, pulse]);
...
export default function Compass({ onQuit }: { onQuit?: () => void }) {
  ...
  const stableHeading = compassStable ? deviceHeading! : (gpsOk ? geo!.headingFromGPS! : null);
  ...
  const getRot = (uid: string, b: number) => {
    if (stableHeading == null) return angleSmoothRef.current[uid] ?? 0;
  ...
  const hintFor = (b:number) => {
    ...
    let diff = (b - stableHeading + 540) % 360 - 180;
      ...
      <div className="flex-1 grid place-items-center">
        <div className="relative w-[320px] h-[320px] rounded-full border border-slate-700">
          <div
            ...
            onMouseDown={()=>{ pressT.current = Date.now(); }}
            onMouseUp={()=>{ if (Date.now()-pressT.current>500) setShowDiag(v=>!v); }}
            onTouchStart={()=>{ pressT.current = Date.now(); }}
            onTouchEnd={()=>{ if (Date.now()-pressT.current>500) setShowDiag(v=>!v); }}
          >N</div>
          ...
          {others.map(o => {
            ...
            const showDirText = deviceHeading == null && showHint;
            ...
            return (
              <div key={o.uid} className="absolute left-1/2 top-1/2" style={{ transform: `translate(-50%,-50%) rotate(${rot}deg)` }}>
                ...
                {/* Minimal label (name · distance · last seen) + avatar */}
                <div className="absolute -top-12 left-1/2 -translate-x-1/2 text-sm" style={{ color: o.member.color }}>
                  <div className="px-2 py-0.5 rounded-full bg-black/40 backdrop-blur flex items-center gap-1">
                    {o.member.avatarId && (
                      <span className="inline-flex items-center justify-center rounded-full bg-slate-900/50" style={{ width: 18, height: 18 }}>
          ...
          <div
            ...
            style={{ width: 76, height: 76, borderColor: me?.color || '#475569', boxShadow: '0 0 0 2px rgba(0,0,0,0.4)', background: '#0f172a' }}
          >
            {me?.avatarId ? <AvatarIcon id={me.avatarId} size={56} /> : <div className="text-xs text-slate-300">You</div>}
          </div>
        </div>
      </div>

      {/* bottom chips + vibration toggle (two rows) */}
      <div className="py-2">
        ...
        {focused ? (
          <div className="flex items-center justify-between">
            <span className="text-xs text-slate-400">
              Ciblé&nbsp;:&nbsp;<span className="text-slate-200">{others.find(o=>o.uid===focused)?.member.name || '—'}</span>
            </span>
            <div className="flex items-center gap-2">
              ...
              {cuesOn && typeof navigator.vibrate !== 'function' && (
                <span className="text-[11px] text-slate-400">(vibration non supportée)</span>
              )}
            </div>
          </div>
        ) : null}
