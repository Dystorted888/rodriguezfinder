...
function useWakeLock(active:boolean){
  useEffect(()=>{
    let lock:any=null; let released=false;
    async function run(){
      try { if('wakeLock' in navigator && active){ lock = await (navigator as any).wakeLock.request('screen'); }
      } catch{}
    }
    run();
    return ()=>{ released=true; if(lock) try{ lock.release(); }catch{} };
  },[active]);
}
...
export default function Compass(){
  ...
  useEffect(()=>{
    ...
    const expireAt = new Date(Date.now() + 12*60*60*1000); // 12h
  ...
  const rel = (b:number) => {
    ...
    const r = (b - heading + 360) % 360; // angle to rotate arrow
    return r;
  };
  ...
  return (
    <div className="p-4 h-full flex flex-col">
      <div className="flex items-center justify-between">
        <div className="text-sm text-slate-400">Group: <span className="font-mono">{groupId}</span></div>
      ...
      <div className="flex gap-2 overflow-x-auto py-2">
        {others.map(o=> (
          <button key={o.uid} onClick={()=>setFocused(focused===o.uid? null : o.uid)} className="px-3 py-2 rounded-2xl bg-slate-800 text-sm" style={{ border: focused===o.uid? `2px solid ${o.member.color}`:'2px solid transparent' }}>
